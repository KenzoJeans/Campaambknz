...
- Quitar el botón "Cargar por defecto".
- Añadir en la selección de "Fuente de datos" una opción que cargue directamente
  la hoja pública especificada (link proporcionado).
- Mantener botón de "Refrescar datos" que respete la opción seleccionada.
- Filtros por rango de fecha y por área administrativa.
- Uso de st.session_state para controlar recargas.
"""
...
# ──────────────────────────────────────────────
PUBLIC_SHEET_LINK = "https://docs.google.com/spreadsheets/d/157VmpJo9qvuKDmx12yya2E1caGa28HB4Kxd3-EeY_G8/edit?usp=sharing"

def ensure_csv_export(url: str) -> str:
    """Asegura que la URL de Google Sheets apunte al CSV exportable."""
...
DEFAULT_URL = ensure_csv_export(PUBLIC_SHEET_LINK)
...
with st.sidebar:
    ...
    modo = st.radio(
        ...
        ["📎 Subir archivo", "🔗 URL Google Sheets", "🔗 Hoja pública (predeterminada)"]
    )

    uploaded = None
    gs_url = ""
    # Determinar data_source según la opción seleccionada
    if modo == "📎 Subir archivo":
        ...
        # No asignamos automáticamente a session_state aquí; se hará al presionar Refrescar
    elif modo == "🔗 URL Google Sheets":
        gs_url = st.text_input(
            "URL de Google Sheets:",
            placeholder="https://docs.google.com/spreadsheets/d/...",
            help="La hoja debe estar publicada (Archivo → Publicar en la web → CSV).",
        )
    else:  # Hoja pública (predeterminada)
        st.markdown("Se usará la hoja pública proporcionada como fuente de datos.")
        st.markdown(f"- **Link público:** {PUBLIC_SHEET_LINK}")
        # No asignar aún a session_state hasta que el usuario presione Refrescar,
        # para evitar recargas automáticas no deseadas.
    ...
    st.markdown("---")
    # Botón de refresco: prioriza archivo subido, luego URL ingresada, luego la hoja pública
    if st.button("🔄 Refrescar datos"):
        if modo == "📎 Subir archivo" and uploaded is not None:
            st.session_state["data_source"] = uploaded
        elif modo == "🔗 URL Google Sheets" and gs_url and gs_url.strip():
            st.session_state["data_source"] = gs_url.strip()
        else:
            # Si el usuario eligió la opción "Hoja pública (predeterminada)" o no dio input,
            # usar la URL pública por defecto
...
# ──────────────────────────────────────────────
# Si ya hay data_source en session_state, la usamos. Si no, intentamos inferir
# desde los inputs actuales del sidebar (sin forzar carga).
if st.session_state.get("data_source") is None:
    # Intentar inferir sin recargar automáticamente:
    try:
        if 'uploaded' in locals() and uploaded is not None:
            ...
            # No hay nada: dejar None para que el usuario presione Refrescar
            st.session_state["data_source"] = None
    except Exception:
        st.session_state["data_source"] = None

# Si no hay fuente definida, mostrar instrucción y detener ejecución
if st.session_state["data_source"] is None:
    st.info("👈 Selecciona la fuente de datos en el panel lateral y presiona 'Refrescar datos'.\n\nSi eliges 'Hoja pública (predeterminada)' y presionas 'Refrescar datos', se cargará la hoja pública proporcionada.")
    st.stop()
