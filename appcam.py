...
# ──────────────────────────────────────────────
# ESTILOS CSS (TEMA OSCURO + INFOGRAFÍA)
# ──────────────────────────────────────────────
# NOTA: En Python f-strings, las llaves de CSS deben escribirse dobles {{ y }}
st.markdown(f"""
<style>
    ...
    /* --------------------------------------------------- */
    /* ESTILOS NUEVOS DE LA INFOGRAFÍA (FLYER)             */
    /* --------------------------------------------------- */
    .flyer-container {{ font-family: 'Inter', sans-serif; margin-top: 10px; }}
    .flyer-impact-banner {{
    ...
    .impact-stat {{ font-size: 2.2rem; font-weight: 800; color: #fcd34d; }}
    .impact-label {{ font-size: 0.95rem; color: #f0fdf4; text-transform: uppercase; letter-spacing: 0.5px; }}
    
    .flyer-grid {{
        display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr));
        gap: 15px; margin-bottom: 20px;
    }}
    .flyer-card {{ padding: 18px; border-radius: 12px; color: #1f2937; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }}
    .card-green {{ background-color: #d1fae5; border-left: 6px solid #10b981; }}
    .card-blue {{ background-color: #dbeafe; border-left: 6px solid #3b82f6; }}
    .card-amber {{ background-color: #fef3c7; border-left: 6px solid #f59e0b; }}
    
    .card-icon {{ font-size: 1.8rem; margin-bottom: 6px; }}
    .card-title {{ font-weight: bold; font-size: 1.1rem; margin-bottom: 4px; }}
    .card-value {{ font-size: 1.6rem; font-weight: 800; margin-bottom: 6px; }}
    .card-desc {{ font-size: 0.85rem; color: #4b5563; line-height: 1.3; }}
    ...
    .winners-title {{ font-size: 1.2rem; font-weight: bold; margin-bottom: 15px; color: #111827; }}
    .winners-grid {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 12px; }}
    .winner-box {{ background: #f9fafb; padding: 12px; border-radius: 8px; border: 1px solid #e5e7eb; text-align: center; }}
    .winner-category {{ font-size: 0.8rem; color: #6b7280; font-weight: 600; }}
    .winner-name {{ font-size: 1.05rem; font-weight: bold; color: #111827; margin: 4px 0; }}
    .winner-score {{ font-size: 0.95rem; color: #059669; font-weight: bold; }}
    
    .flyer-footer {{
        background-color: #ecfdf5; color: #065f46; padding: 14px; border-radius: 8px;
        text-align: center; font-weight: 600; border: 1px dashed #10b981;
    }}
</style>
...
def clean_weight(series: pd.Series) -> pd.Series:
    ...
    s = s.replace(["No Participa", "No participa", "no participa", "", "nan", "None", "NaN"], "0")
...
def gs_url_to_csv(url: str) -> str:
    ...
    if "/edit?usp=sharing" in url: return url.replace("/edit?usp=sharing", "/export?format=csv")
...
def _normalize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df.columns:
        cl = col.lower()
        if "marca temporal" in cl or "timestamp" in cl: rename_map[col] = "timestamp"
        elif "fecha" in cl and "marca" not in cl: rename_map[col] = "fecha"
        elif "grupo" in cl and "participac" in cl: rename_map[col] = "grupo"
        elif "área" in cl or "area" in cl or "administrativo" in cl: rename_map[col] = "area_admin"
        elif "nombre" in cl or ("persona" in cl and "operac" in cl): rename_map[col] = "nombre_persona"
        elif "tienda" in cl and "pertenece" in cl: rename_map[col] = "tienda"
        elif "botellas" in cl: rename_map[col] = "botellas_kg"
        elif "tapas" in cl: rename_map[col] = "tapas_kg"
        elif "aceite" in cl: rename_map[col] = "aceite_kg"
    df = df.rename(columns=rename_map)

    for c in ["grupo","area_admin","nombre_persona","tienda","botellas_kg","tapas_kg","aceite_kg"]:
    ...
    if fecha_col:
        ...
        if tasa_fallo > 0.3:
            ...
            if alterna.isna().mean() < tasa_fallo: parsed = alterna
        df["fecha_dt"] = parsed
    else:
        df["fecha_dt"] = pd.NaT
...
def apply_dark_theme(fig, height=None, legend_bottom=True):
    layout_kwargs = dict(
        template="plotly_dark", paper_bgcolor=DARK_PANEL, plot_bgcolor=DARK_PANEL,
    ...
    if height: layout_kwargs["height"] = height
...
def top_n_df(df_in: pd.DataFrame, col_label: str, col_value: str, top_n: int = 10) -> pd.DataFrame:
    return (df_in.groupby(col_label, as_index=False)[col_value]
            .sum().query(f"{col_value} > 0")
            .sort_values(col_value, ascending=False).head(top_n))
...
def top10_bar(df_in, col_label, col_value, title, color, top_n=10):
    ...
    if df_plot.empty:
        fig = go.Figure()
        fig.update_layout(title=title + " — Sin datos", annotations=[dict(
                text="Sin registros para los filtros actuales", xref="paper", yref="paper", x=0.5, y=0.5,
                showarrow=False, font=dict(size=13, color=DARK_MUTED))])
        return apply_dark_theme(fig)
    fig = go.Figure(go.Bar(
        x=df_plot[col_value], y=df_plot[col_label], orientation="h",
        marker=dict(color=df_plot[col_value], colorscale=[[0, DARK_BORDER],[1, color]],
                    showscale=False, line=dict(color=DARK_PANEL, width=0.5)),
        text=[f"{v:.1f} kg" for v in df_plot[col_value]],
        textposition="outside", textfont=dict(color=DARK_TEXT),
    ...
    fig.update_layout(
        ...
        margin=dict(l=10, r=70, t=50, b=30), height=max(300, top_n * 38), yaxis=dict(tickfont=dict(size=11))
    )
...
def _tabla_pdf(data, col_widths=None):
    ...
    tabla.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, 0), PDF_GREEN), ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
        ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"), ("FONTNAME", (0, 1), (-1, -1), "Helvetica"),
        ("FONTSIZE", (0, 0), (-1, -1), 8.5), ("ROWBACKGROUNDS", (0, 1), (-1, -1), [colors.white, PDF_GREEN_LIGHT]),
        ("GRID", (0, 0), (-1, -1), 0.4, PDF_BORDER), ("ALIGN", (1, 0), (-1, -1), "CENTER"),
...
def generar_pdf_reporte(df, df_op, df_tienda, df_admin, df_adm_gpo, top_n, filtro_texto):
    ...
    titulo_style = ParagraphStyle("TituloVerde", parent=styles["Title"], textColor=PDF_GREEN)
    heading_style = ParagraphStyle("SubtituloVerde", parent=styles["Heading2"], textColor=PDF_GREEN, spaceBefore=14, spaceAfter=6)
    ...
    total_botellas, total_tapas, total_aceite = df["botellas_kg"].sum(), df["tapas_kg"].sum(), df["aceite_kg"].sum()
    total_kg = total_botellas + total_tapas + total_aceite

    elementos.append(Paragraph("Resumen General", heading_style))
    kpi_data = [
        ["Métrica", "Valor"], ["Participantes", f"{len(df)}"],
        ["Botellas con Amor (kg)", f"{total_botellas:.1f}"], ["Tapas para Sanar (kg)", f"{total_tapas:.1f}"],
        ["Aceite Green Fuel (kg)", f"{total_aceite:.1f}"], ["Total recolectado (kg)", f"{total_kg:.1f}"]
    ]
    ...
    def _agregar_ranking(titulo, df_in, col_label, col_value):
        ...
        for i, row in enumerate(top_df.itertuples(index=False), start=1):
            data.append([str(i), str(getattr(row, col_label)), f"{getattr(row, col_value):.1f}"])
    ...
    df_admin_et["etiqueta"] = df_admin_et.apply(lambda r: r["nombre_persona"] if r["nombre_persona"] != "N/A" else r["area_admin"], axis=1)
...
with st.sidebar:
    ...
    modo = st.radio("Selecciona el modo de carga:", ["🔗 Base de datos oficial", "🌐 Otra URL de Google Sheets", "📎 Subir archivo"], index=0)
    data_key, data_bytes, data_fname = None, None, None
    ...
    if modo == "🔗 Base de datos oficial":
        ...
        gs_url = st.text_input("URL de Google Sheets:", placeholder="https://docs.google.com/spreadsheets/d/...")
        if gs_url.strip(): data_key = gs_url.strip()
        else: st.info("Ingresa la URL para continuar.")
    else:
        uploaded = st.file_uploader("Sube el archivo (CSV o Excel):", type=["csv","xlsx","xls"])
        if uploaded:
            data_bytes, data_fname, data_key = uploaded.getvalue(), uploaded.name, uploaded.name
    ...
    if usar_filtro:
        ...
        with col_a: fecha_inicio = st.date_input("Desde:", value=date(2025, 1, 1))
        with col_b: fecha_fin = st.date_input("Hasta:", value=date.today())
        if fecha_inicio > fecha_fin: st.error("⚠️ La fecha de inicio debe ser anterior a la de fin.")
...
try:
    with st.spinner("⏳ Cargando datos..."):
        df_raw = load_from_bytes(data_bytes, data_fname) if data_bytes else load_from_url(data_key)
except Exception as e:
    st.error(f"❌ Error al cargar los datos: {e}")
    st.stop()

if usar_filtro and fecha_inicio and fecha_fin and fecha_inicio <= fecha_fin:
    mask = df_raw["fecha_dt"].notna() & (df_raw["fecha_dt"].dt.date >= fecha_inicio) & (df_raw["fecha_dt"].dt.date <= fecha_fin)
    df = df_raw[mask].copy()
    badge_txt = f"🗓️ Filtro activo: {fecha_inicio.strftime('%d/%m/%Y')} → {fecha_fin.strftime('%d/%m/%Y')} &nbsp;·&nbsp; <b>{len(df)}</b> de <b>{len(df_raw)}</b> registros"
    filtro_texto_pdf = f"Filtro de fecha: {fecha_inicio.strftime('%d/%m/%Y')} – {fecha_fin.strftime('%d/%m/%Y')}"
else:
    df = df_raw.copy()
...
# ══════════════════════════════════════════════
# 0. INFOGRAFÍA / KPIs GENERALES (DINÁMICO)
# ══════════════════════════════════════════════
...
# Calcular el top 1 dinámicamente para el cuadro de honor
def get_top_1(df_sub, group_col, val_col):
    if df_sub.empty or df_sub[val_col].sum() == 0:
        return "Pendiente", 0
    top = df_sub.groupby(group_col)[val_col].sum().sort_values(ascending=False)
    if top.empty:
        return "Pendiente", 0
    return top.index[0], top.iloc[0]

top_op_botellas_name, top_op_botellas_val = get_top_1(df_op, "nombre_persona", "botellas_kg")
top_op_tapas_name, top_op_tapas_val       = get_top_1(df_op, "nombre_persona", "tapas_kg")
top_tienda_name, top_tienda_val           = get_top_1(df_tienda, "tienda", "botellas_kg")

# Inyectar las variables de Python directamente en el HTML de la infografía
html_infografia = f"""
<div class="flyer-container">
    <div class="flyer-impact-banner">
        ...
        <div>
            <div class="impact-stat">{total_participantes}</div>
    ...
    <div class="flyer-grid">
        ...
        <div class="flyer-card card-blue">
            ...
            <div class="card-desc">Apoyando fuertemente tratamientos de salud infantil con la recolección.</div>
        </div>
        <div class="flyer-card card-amber">
            <div class="card-icon">🛢️</div>
            <div class="card-title">Aceite Green Fuel</div>
            <div class="card-value">{total_aceite:.1f} kg</div>
            <div class="card-desc">Protegimos el agua evitando contaminación por aceites usados.</div>
        </div>
    </div>
    ...
    <div class="flyer-winners-section">
        ...
        <div class="winners-grid">
            <div class="winner-box">
                ...
                <div class="winner-name">{top_op_botellas_name}</div>
                <div class="winner-score">{top_op_botellas_val:.1f} kg</div>
            </div>
            <div class="winner-box">
                <div class="winner-category">🥇 Planta (Tapas)</div>
                <div class="winner-name">{top_op_tapas_name}</div>
                <div class="winner-score">{top_op_tapas_val:.1f} kg</div>
            </div>
            <div class="winner-box">
                <div class="winner-category">🏬 Tienda Líder (Botellas)</div>
                <div class="winner-name">{top_tienda_name}</div>
                <div class="winner-score">{top_tienda_val:.1f} kg</div>
            </div>
        </div>
    </div>
...
# Renderizamos la infografía generada
...
with c1:
    ...
    fig_pie = px.pie(part_grupo, values="Participantes", names="Grupo", title="% Participación por Grupo", color_discrete_sequence=PALETTE_MULTI, hole=0.42)
...
with c2:
    ...
    df_gkg_m["Campaña"] = df_gkg_m["Campaña"].map({"botellas_kg":"Botellas con Amor","tapas_kg":"Tapas para Sanar","aceite_kg":"Aceite Green Fuel"})
    fig_bg = px.bar(df_gkg_m.query("kg > 0"), x="grupo", y="kg", color="Campaña", barmode="group", title="Kg recolectados por Grupo y Campaña",
                    color_discrete_map={"Botellas con Amor":COLOR_BOTELLAS,"Tapas para Sanar":COLOR_TAPAS,"Aceite Green Fuel":COLOR_ACEITE}, text_auto=".1f")
...
c1, c2 = st.columns(2)
with c1:
    camp_tot = pd.DataFrame({"Campaña":["Botellas con Amor","Tapas para Sanar","Aceite Green Fuel"], "kg":[total_botellas, total_tapas, total_aceite]})
    fig_camp = px.pie(camp_tot, values="kg", names="Campaña", title="Distribución de kg por Campaña",
                      color_discrete_map={"Botellas con Amor":COLOR_BOTELLAS,"Tapas para Sanar":COLOR_TAPAS,"Aceite Green Fuel":COLOR_ACEITE}, hole=0.42)
    apply_dark_theme(fig_camp, height=360)
    st.plotly_chart(fig_camp, use_container_width=True)

with c2:
    df_pc = pd.DataFrame({"Campaña":["Botellas con Amor","Tapas para Sanar","Aceite Green Fuel"],
                          "Participantes":[(df["botellas_kg"]>0).sum(),(df["tapas_kg"]>0).sum(),(df["aceite_kg"]>0).sum()]})
    fig_pcamp = px.bar(df_pc, x="Campaña", y="Participantes", title="Participantes por Campaña", color="Campaña",
                       color_discrete_map={"Botellas con Amor":COLOR_BOTELLAS,"Tapas para Sanar":COLOR_TAPAS,"Aceite Green Fuel":COLOR_ACEITE}, text="Participantes")
...
if df["fecha_dt"].dropna().dt.date.nunique() > 1:
    ...
    df_time = df.groupby(df["fecha_dt"].dt.date)[["botellas_kg","tapas_kg","aceite_kg"]].sum().reset_index().rename(columns={"fecha_dt":"Fecha"})
    df_tm = df_time.melt(id_vars="Fecha", var_name="Campaña", value_name="kg")
    df_tm["Campaña"] = df_tm["Campaña"].map({"botellas_kg":"Botellas con Amor","tapas_kg":"Tapas para Sanar","aceite_kg":"Aceite Green Fuel"})
    fig_time = px.line(df_tm, x="Fecha", y="kg", color="Campaña", markers=True, title="Recolección acumulada por Fecha",
                       color_discrete_map={"Botellas con Amor":COLOR_BOTELLAS,"Tapas para Sanar":COLOR_TAPAS,"Aceite Green Fuel":COLOR_ACEITE})
...
c1, c2 = st.columns(2)
with c1: st.plotly_chart(top10_bar(df_op, "nombre_persona", "botellas_kg", f"🥇 Top {top_n} Operadores — Botellas con Amor", COLOR_BOTELLAS, top_n), use_container_width=True)
with c2: st.plotly_chart(top10_bar(df_op, "nombre_persona", "tapas_kg", f"🥇 Top {top_n} Operadores — Tapas para Sanar", COLOR_TAPAS, top_n), use_container_width=True)
...
c1, c2 = st.columns(2)
with c1: st.plotly_chart(top10_bar(df_tienda, "tienda", "botellas_kg", f"🥇 Top {top_n} Tiendas — Botellas con Amor", COLOR_BOTELLAS, top_n), use_container_width=True)
with c2: st.plotly_chart(top10_bar(df_tienda, "tienda", "tapas_kg", f"🥇 Top {top_n} Tiendas — Tapas para Sanar", COLOR_TAPAS, top_n), use_container_width=True)
...
df_aceite_all["etiqueta"] = df_aceite_all.apply(lambda r: r["nombre_persona"] if r["nombre_persona"] != "N/A" else (r["area_admin"] if r["area_admin"] != "N/A" else r["tienda"]), axis=1)
st.plotly_chart(top10_bar(df_aceite_all, "etiqueta", "aceite_kg", f"🛢️ Top {top_n} — Aceite Green Fuel", COLOR_ACEITE, top_n), use_container_width=True)
...
df_admin["etiqueta"] = df_admin.apply(lambda r: r["nombre_persona"] if r["nombre_persona"] != "N/A" else r["area_admin"], axis=1)
c1, c2 = st.columns(2)
with c1: st.plotly_chart(top10_bar(df_admin, "etiqueta", "botellas_kg", f"🥇 Top {top_n} Admin — Botellas con Amor", COLOR_BOTELLAS, top_n), use_container_width=True)
with c2: st.plotly_chart(top10_bar(df_admin, "etiqueta", "tapas_kg", f"🥇 Top {top_n} Admin — Tapas para Sanar", COLOR_TAPAS, top_n), use_container_width=True)
...
df_adm_gpo = df_admin[df_admin["grupo_admin"].notna()].groupby("grupo_admin")[["botellas_kg","tapas_kg"]].sum().reset_index()
...
if not df_adm_gpo.empty:
    df_gm = df_adm_gpo.melt(id_vars=["nombre_grupo","total_kg"], value_vars=["botellas_kg","tapas_kg"], var_name="Campaña", value_name="kg")
    df_gm["Campaña"] = df_gm["Campaña"].map({"botellas_kg":"Botellas con Amor","tapas_kg":"Tapas para Sanar"})
    fig_gs = px.bar(df_gm.query("kg > 0"), x="kg", y="nombre_grupo", orientation="h", color="Campaña", barmode="stack", title="Ranking Grupos Admin (Botellas + Tapas)",
                    color_discrete_map={"Botellas con Amor":COLOR_BOTELLAS,"Tapas para Sanar":COLOR_TAPAS}, text_auto=".1f")
...
st.download_button(label="⬇️ Descargar reporte PDF", data=pdf_bytes, file_name=f"reporte_campanas_{date.today().isoformat()}.pdf", mime="application/pdf", type="primary")
...
df_heat = df_admin.groupby("area_admin")[["botellas_kg","tapas_kg","aceite_kg"]].sum().rename(columns={"botellas_kg":"Botellas con Amor","tapas_kg":"Tapas para Sanar","aceite_kg":"Aceite Green Fuel"}).query("`Botellas con Amor` + `Tapas para Sanar` + `Aceite Green Fuel` > 0")
if not df_heat.empty:
    fig_heat = px.imshow(df_heat, text_auto=".1f", title="Mapa de Calor — Kg por Área y Campaña", color_continuous_scale=[[0, DARK_PANEL], [1, ACCENT_GREEN]], aspect="auto")
...
with st.expander("Ver / Descargar tabla completa", expanded=False):
    col_show = [c for c in ["fecha","grupo","area_admin","nombre_persona","tienda","botellas_kg","tapas_kg","aceite_kg"] if c in df.columns]
    st.dataframe(df[col_show].rename(columns={"fecha":"Fecha","grupo":"Grupo","area_admin":"Área","nombre_persona":"Persona","tienda":"Tienda","botellas_kg":"Botellas (kg)","tapas_kg":"Tapas (kg)","aceite_kg":"Aceite (kg)"}), use_container_width=True, height=300)
    st.download_button("⬇️ Descargar CSV filtrado", data=df[col_show].to_csv(index=False).encode("utf-8-sig"), file_name="campanas_ambientales_filtrado.csv", mime="text/csv")
